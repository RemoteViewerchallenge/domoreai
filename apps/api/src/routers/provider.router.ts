...
import { db } from '../db/index.js'; // Your Prisma client
...
export const providerRouter = createTRPCRouter({
  ...
  fetchAndNormalizeModels: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const rawModelList: any[] = await getModelsFromProvider(
...
const decrypt = (text: string | null | undefined) => text?.replace('ENCRYPTED:', '') || '';
 ...
 */
async function getModelsFromProvider(type: string, baseURL: string, apiKey?: string): Promise<any[]> {
