...
throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: `Git Log failed: ${error.message}` });
}
}
...
throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: `Git Commit failed: ${error.message}` });
}
}
}
