...
export class AgentService {
  ...
  async generateQuery(input: { userPrompt: string, targetTable?: string, roleName?: string }) {
    ...
    if (roleName) {
      ...
      if (!role) {
        console.warn(`[AgentService] Role "${roleName}" not found; proceeding without role-specific settings.`);
      }
    }
    ...
    try {
      ...
      console.warn(`[AgentService] Dynamic model selection failed for role ${roleName}:`, e);
    }

    // Fallback if dynamic selection fails
    if (!selectedModel) {
      console.log(`[AgentService] Falling back to any enabled model for ${roleName}`);
    ...
    console.log(`[AgentService] Generating SQL using model: ${selectedModel.modelId} (${selectedModel.providerId})`);
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        console.warn(`[AgentService] Model generation failed for ${selectedModel.modelId} (${selectedModel.providerId}): ${errMsg}`);
        ...
        if (/invalid model|not found|invalid model id/i.test(errMsg)) {
          ...
          try {
            ...
            console.warn('[AgentService] Failed to persist model/provider failure:', recErr);
          }
          ...
          try {
            ...
            if (!selectedModel) {
              console.warn('[AgentService] No alternative model available after failure');
              throw new Error('No alternative models available');
            }
            console.log(`[AgentService] Retrying with new model: ${selectedModel.modelId} (${selectedModel.providerId})`);
          } catch (selectErr) {
            console.warn('[AgentService] Failed to select alternative model:', selectErr);
            ...
            try {
              ...
              console.log(`[AgentService] Retrying with fallback: ${selectedModel.modelId} (${selectedModel.providerId})`);
              continue;
            } catch (fallbackErr) {
              console.warn('[AgentService] Fallback selection failed:', fallbackErr);
            }
          }
        } else {
          throw err;
