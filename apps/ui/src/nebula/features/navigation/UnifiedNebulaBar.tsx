...
export const UnifiedNebulaBar = ({ aiOpen, setAiOpen, onToggleTheme, themeOpen }: UnifiedNebulaBarProps) => {
  ...
  return (
    <div className="flex-none h-10 bg-[var(--bg-secondary)] border-b border-[var(--border-color)] flex items-center justify-between select-none px-2 gap-2">
      ...
      {/* 3. COLUMN CONTROLS + AI COMMAND CENTER (Right) */}
      <div className="flex items-center gap-2 h-full">
        {/* Column Controls (only show in Workbench) */}
        {isWorkbench && (
          <>
            <div className="flex items-center gap-1 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-sm px-2 py-1">
              <LayoutGrid size={12} className="text-[var(--text-muted)]" />
              <button
                onClick={() => setColumns(Math.max(1, columns - 1))}
                disabled={columns <= 1}
                className="w-6 h-6 flex items-center justify-center hover:bg-[var(--bg-secondary)] disabled:opacity-30 disabled:hover:bg-transparent text-[var(--text-primary)] rounded transition-all text-sm font-bold"
                title="Decrease columns"
              >
                −
              </button>
              <span className="text-[10px] font-mono text-[var(--text-primary)] w-3 text-center">{columns}</span>
              <button
