...
export const TopSummaryBar = ({ cardCount, onSpawnCard }: TopSummaryBarProps) => {
  const [isExpanded, setIsExpanded] = useState(false);

  return (
    <div
      className={`flex-none bg-zinc-950 border-b border-zinc-900 transition-all duration-200 overflow-hidden ${
        isExpanded ? 'h-16' : 'h-6'
      }`}
