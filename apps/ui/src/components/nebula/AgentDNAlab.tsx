...
import {
  ...
  Fingerprint, Cpu, Shield, Globe, Wrench, Zap
} from 'lucide-react';
...
import { DynamicCapabilityGrid } from './DynamicCapabilityGrid.js';
import { cn } from '../../lib/utils.js';

type LabTab = 'identity' | 'cortex' | 'governance' | 'context' | 'tuning' | 'tools' | 'capabilities';
    ...
    <div className="flex h-full w-full bg-[var(--bg-background)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex px-6 border-b border-[var(--border-color)] bg-[var(--bg-secondary)]/10">
          ...
          <DNATab active={activeTab === 'capabilities'} onClick={() => setActiveTab('capabilities')} icon={Zap} label="Capabilities" color="yellow" />
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                <ModelFilter
                  ...
                  criteria={{
                    ...
                    // Infer mode from boolean flags
                    mode: (formData.needsImageGeneration ? 'IMAGE_GEN' :
                      formData.needsVision ? 'VISION' :
                        formData.needsReasoning ? 'REASONING' :
                          formData.needsCoding ? 'REASONING' :
                            formData.needsUncensored ? 'CHAT' : 'CHAT') as any,
                    preferences: {
                  ...
                  onChange={(crit) => {
                    // When filter changes, update the booleans
                    setFormData({
                      ...formData,
                      minContext: crit.minContext,
                      maxContext: crit.maxContext,
                      needsVision: crit.mode === 'VISION',
                      needsReasoning: crit.mode === 'REASONING' || !!crit.preferences?.reasoning,
                      needsImageGeneration: crit.mode === 'IMAGE_GEN',
                      needsUncensored: !!crit.preferences?.uncensored,
                      needsCoding: crit.mode === 'REASONING' // Assume reasoning includes coding
                    });
                  }}
                />
              </div>
